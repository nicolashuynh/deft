...
class HHCARTNode:
    ...
    def __init__(self, depth, labels, **kwargs):  # Defining the node structure
        ...
        self.is_leaf = kwargs.get(
            "is_leaf", False
        )  # 'is_leaf' is set to 'False' for internal nodes
        self._split_rules = kwargs.get(
            "split_rules", None
        )  # 'split_rules' is set to 'None'
...
class HouseHolderCART(BaseEstimator):
    ...
    def _terminate(self, X, y, cur_depth):  # termination conditions
        ...
        """Handle terminate."""
        if (
            self._max_depth != None and cur_depth == self._max_depth
    ...
    def _generate_node(self, X, y, cur_depth):
        ...
        if self._terminate(X, y, cur_depth):
            ...
            if (check_ > self.tau).sum() > 0:
                ...
                impurity_house, sr_house, left_indices_house, right_indices_house = (
...
class HHCartClassifier(ClassifierMixin, HouseHolderCART):
    def __init__(
        self, impurity, segmentor, max_depth=50, min_samples_split=2, **kwargs
    ):
        """Initialize the instance."""
        super().__init__(
            impurity=impurity,
